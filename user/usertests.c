...
truncate3(char *s)
{
  ...
  wait(&xstatus, (char*) exit_msg_buf);
  unlink("truncfile");
  exit(xstatus, "xstatus\n");
}
...
exectest(char *s)
{
  ...
  if (wait(&xstatus, (char*) exit_msg_buf) != pid) {
...
exitwait(char *s)
{
  ...
  for(i = 0; i < 100; i++){
    ...
    if(pid){
      int xstate;
      if(wait(&xstate, (char*) exit_msg_buf) != pid){
...
reparent(char *s)
{
  ...
  for(int i = 0; i < 200; i++){
    ...
    if(pid){
      if(wait(0, (char*) exit_msg_buf) != pid){
        printf("%s: wait wrong pid\n", s);
        exit(1, "status 1\n");
      }
    } else {
